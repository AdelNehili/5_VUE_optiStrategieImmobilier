...
def setupFamilyHouse():
    ...
    bizarre_male = Bizarre("ElTortueAdaptor", 30, human_Expenses, personal_Expenses, 1670, 10000, "g-", 0.10, 8)
...
def newStrategy_come_back(bizarre_male, bizarre_femelle, salaryPartition, house):
    if isinstance(bizarre_male, Human) and isinstance(bizarre_femelle, Human):
        bizarre_male.setEconomiesPartition(salaryPartition)
#        bizarre_male.setSalary(bizarre_male.getSalary())
        bizarre_male.setPersonalExpenses([0])

        bizarre_femelle.setSalary(800)
        bizarre_femelle.setEconomiesPartition(salaryPartition)
        bizarre_femelle.setPersonalExpenses([0])

    #Les allocations rediminuent s'il tout le monde travaille
    if isinstance(house,Maison):
        for child in house.getFamily():
            if isinstance(child,Child):
                child.setSalary(130)
...
if test1:
    ...
    maison.livingMonths(10 * 12, isDisplayed)
    newStrategy_come_back(bizarre_male, bizarre_femelle,0,maison)
